...
int handle_enter_gettimeofday(struct trace_event_raw_sys_enter *ctx)
{
    ...
    struct __kernel_old_timeval *time_p = (struct __kernel_old_timeval *) ctx->args[0]; 
    if (time_p == NULL)
    {
        return 0;
    }
...
int handle_exit_gettimeofday(struct trace_event_raw_sys_exit *ctx)
{
    ...
    long success = bpf_probe_read_user(&retval, sizeof(struct __kernel_old_timeval), time_p);
...
int handle_exit_clock_gettime(struct trace_event_raw_sys_exit *ctx)
{
    ...
    long success = bpf_probe_read_user(&retval, sizeof(struct timespec64), time_p);
